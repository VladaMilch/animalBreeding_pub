...
names(mousebreeding)
mousebreeding <- lapply(mousebreeding, FUN = function(df){
  df = data.frame(df)
  df$pups_born <- as.numeric(df$pups_born)
  df$pups_beforeW <- as.numeric(df$pups_beforeW)
  df$weaned_f <- as.numeric(df$weaned_f)
  df$weaned_m <- as.numeric(df$weaned_m)
  return(df)
})
...
for (ii in 1:length(mousebreeding)){
  ...
  hist(mousebreeding[[ii]]$weaned_m, breaks = 20, col = "blue")
}

head(mousebreeding[[2]])
names(mousebreeding)
```
...
head(table_list_clean[[1]])
save(table_list_clean, 
     file = "./figures/figsSupplementary/litter_size_data_from_pdfs.Rdata")
```
