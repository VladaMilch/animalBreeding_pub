...
image(t(neededmat))
contour(1:maxgood,1:maxgood,neededmat,nlevels=15,ylab = "required number of animals for experiment 0.25 probability", xlab = "required number of animals for experiment 0.25 probability.")
...
ALL
write.csv(ALL, file = "ALLrequiredno025twogroups")
...
MENDEL <- c(1:15)/0.25
MENDEL
plot(EQUAL, ylim=c(1,85), ylab = "required number of born animals", xlab = "required number of animals for experiment (size group A = size group B)", main = "0.25 genotype probability", pch = 19, cex = 1.2)
points(MENDEL, pch = 2)
